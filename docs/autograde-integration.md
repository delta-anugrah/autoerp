...
| Truck (plate, supplier, class, capacity) | AutoERP | AutoERP → AutoGrade; **pending stubs** AutoGrade → AutoERP |
...
GET /api/resource/Truck?fields=["name","plate_number","plate_normalized","supplier","vehicle_class",
    "disabled","pending","modified"]&filters=[["modified",">","<cursor>"]]
```
...
AutoERP matches by `plate_normalized`; if absent it creates a `Truck` with `pending=1` (new Check
"Perlu Dilengkapi") and returns `{"name": "B 5455 DK", "pending": 1}`. AutoGrade stores `erp_name`.
...
- Submit the ticket. `on_submit`: Plasma / Pihak Ketiga → Purchase Receipt (qty = net after deduction,
  rate = harga, `custom_weighbridge_ticket`, dimensions from the ticket, posting time = `time_in`);
  Inti → Stock Entry Material Receipt against the blok. This is exactly what `pks_04_seed.py::_purchase_receipt`
...
4. Later: automatic Purchase Invoice per supplier per period, supplier rating fed back to AutoGrade
   (`suppliers.rating`), pending-truck notifications.
...
source typing the ticket needs (Plasma → Plasma, Agen TBS → Pihak Ketiga, none → Inti).
...
| `pending` | Check, default 0, in list view + standard filter | 1 = created from the mill (interface B) and not yet completed. Label "Perlu Dilengkapi". Cleared automatically in `validate` when `supplier` and `vehicle_class` are both set. Ticket finalisation (E) does not wait on it, but the Truck list shows it as the backoffice to-do. |
| `source` | Select `Manual` / `AutoGrade`, read-only | Audit only: where the record came from. |

Interface A exposes `plate_normalized`, `pending`, `disabled`, `supplier`, `vehicle_class`, `modified`.
...
- **Do:** Add `plate_normalized` (Data, read-only, unique), `autograde_id` (Data, read-only), `pending`
  (Check, list view + standard filter, label "Perlu Dilengkapi"), `source` (Select Manual/AutoGrade). On
  validate: `plate_normalized = re.sub(r"[^A-Za-z0-9]", "", plate_number).upper()`; clear `pending` when
...
- **Done when:** saving "B1234XY" while "B 1234 XY" exists is rejected; list filter `pending=1` works.
- **Needs:** nothing.
...
- **Do:** Script in palmgrade-api `scripts/reconcile-erp.ts`: export active suppliers and trucks; match
  ...
  (`pending=0`, `source=Manual`); write `erp_name` back. Print unmatched suppliers for a human to resolve.
...
- **Do:** `erpPull.service.ts`, `setInterval` 5 min, single-flight, cloud role only. Page Supplier then Truck
  ...
  `supplier_id`, status from `disabled`/`pending`); mark rows `sync_status='pending'` so `edgeSync` carries them
  to the mill; advance the cursor only after a page fully succeeds.
- **Done when:** a Truck disabled in AutoERP shows inactive on the mill copy within 6 minutes; restarting the
...
- **Done when:** a plate typed at the mill appears in AutoERP as `pending=1` within about a minute of reaching
...
- **Do:** Whitelisted method `autoerp.integrations.autograde.upsert_truck`. Key: `plate_normalized`. If found,
  set `autograde_id` when empty and return it; else create with `pending=1`, `source=AutoGrade`. Return
  `{name, pending}`. Role-checked, idempotent, 4xx on malformed plate.
...
- **Do:** In `truck-list.tsx`, `truck.tsx`, `supplier-form.tsx`: when `erp_name` is set, disable edit and
  ...
  linked / pending in ERP / not linked. `GET /trucks` and `GET /suppliers` must return `erp_name` (AG-1).
- **Done when:** a linked truck cannot be edited from either the cloud or the mill UI; an unlinked one can.
- **Needs:** AG-1, AG-3.

**OPS-3 · docs · Runbook** · S
- **Do:** How to complete a pending truck in AutoERP; how to read and requeue an `error` outbox row; what to do
  when OPS-2 left an exception.
- **Done when:** a backoffice user completes a pending truck end to end from the runbook alone.
- **Needs:** AG-5, FE-1.
...
- **Done when:** replaying the same weighing changes nothing; a weighing for a plate never seen creates a pending
...
- **Do:** `on_submit`: Plasma / Pihak Ketiga → Purchase Receipt (item `tbs_item`, qty = net after deduction,
  rate = harga, `custom_weighbridge_ticket`, dimensions, `set_posting_time`, posting datetime = `time_in`,
  warehouse = company default for TBS); Inti → Stock Entry Material Receipt against the blok. Payloads mirror
...
- **Do:** The ticket form is usable by a human weighbridge operator: truck picker fills plate/class/driver
  ...
  four `status` values and a "Perlu Dilengkapi" filter on Truck.
...
- **Do:** Move Truck, Weighbridge Ticket, Weighbridge Grading, Blok, Kebun, Divisi, Sumber TBS, Sertifikasi,
  Harvester Premi, TBS Grading Rule and the three whitelisted methods into the `AutoERP Integrations` module as
...
back to AutoGrade `suppliers.rating`; notifications for trucks left pending more than a day.
